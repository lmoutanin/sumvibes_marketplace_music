    ...
    <div className="relative min-h-screen bg-gradient-premium"><Navbar />
      <main className="pt-20 flex items-center justify-center min-h-[60vh]">
        <div className="text-center">
          ...
          <Link href="/" className="btn-primary px-6 py-3 rounded-none mt-4 inline-block">Retour à l'accueil</Link>
        </div>
      </main>
    </div>
  );
          ...
          <div className="flex flex-wrap gap-2 mb-8">
            {tabs.map(tab => (
              <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                className={`px-5 py-2.5 rounded-none text-sm font-semibold flex items-center gap-2 ${activeTab === tab.id ? "bg-brand-gold text-brand-purple" : "glass hover:bg-white/10"}`}>
