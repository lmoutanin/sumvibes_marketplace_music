        ...
        <section className="px-6 py-12 md:py-16">
          <div className="mx-auto max-w-7xl">
            ...
            <p className="text-xl text-slate-400 max-w-2xl">
              Explorez des milliers de compositions d&apos;exception. Filtrez par genre, BPM et ambiance.
            </p>
            <div className="flex flex-wrap gap-3 mt-6">
              <Link href="/catalogue/style" className="glass px-4 py-2 rounded-none text-sm hover:bg-white/10">Par Style</Link>
              <Link href="/catalogue/bpm" className="glass px-4 py-2 rounded-none text-sm hover:bg-white/10">Par BPM</Link>
              <Link href="/catalogue/filters/ambiance" className="glass px-4 py-2 rounded-none text-sm hover:bg-white/10">Par Ambiance</Link>
            </div>
          </div>
        </section>
          ...
          <div className="mx-auto max-w-7xl">
            ...
            {(activeFilterCount > 0 || searchQuery) && (
              <div className="flex flex-wrap items-center gap-2 mt-3 px-1">
                ...
                {selectedGenre !== "Tous" && <ActivePill label={selectedGenre} onRemove={() => setSelectedGenre("Tous")} color="gold" />}
          ...
          <div className="mx-auto max-w-7xl">
            ...
            {loading ? (
              ...
              <div className="glass rounded-3xl p-12 text-center">
                <p className="text-xl text-slate-400">Aucune composition trouvée</p>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                {beats.map((beat) => {
                  ...
                  return (
                    <div key={beat.id} className={`glass group rounded-2xl overflow-hidden text-center hover:scale-[1.03] hover:shadow-xl hover:shadow-black/30 transition-all duration-200 relative ${isActive ? "ring-2 ring-brand-gold/50 bg-brand-gold/5" : ""}`}>
                      {/* Full-width cover */}
                      <div className={`relative w-full aspect-square bg-linear-to-br ${gradient}`}>
                        {beat.coverImage
                        ...
                        {isPlaying && (
                          <div className="absolute inset-0 bg-black/55 flex items-end justify-center gap-[3px] pb-3">
                            {[0.1, 0.22, 0.14].map((d, i) => (
                              <span key={i} className="w-[3px] bg-brand-gold rounded-sm" style={{ height: "40%", animation: `eqBar 0.55s ease-in-out ${d}s infinite alternate` }} />
                            ))}
                          </div>
                        )}
                      </div>
                      {/* Info */}
                      <div className="p-4">
                        <h3 className={`font-bold text-base leading-tight mb-0.5 line-clamp-1 transition-colors ${isPlaying ? "text-brand-gold" : "group-hover:text-brand-gold"}`}>{beat.title}</h3>
                        <Link href={`/producers/${beat.seller?.id}`} onClick={(e) => e.stopPropagation()} className="text-xs text-slate-400 mb-3 line-clamp-1 hover:text-brand-gold transition-colors block">{beat.seller?.displayName || beat.seller?.username}</Link>
                        ...
                        <div className="flex items-center justify-between pt-3 border-t border-white/10">
                          <span className="text-brand-gold font-bold text-sm">
                            {Number(beat.basicPrice ?? 0).toFixed(2)} € TTC
                          </span>
                          <div className="flex items-center gap-2">
                            ...
                            {user?.id !== beat.seller?.id && (
                              <button onClick={() => setLicenseTarget(beat)} className="btn-primary px-4 py-2 rounded-lg text-xs font-semibold flex items-center gap-1 shrink-0">
            ...
            <div className="w-[30%] min-w-[200px] flex justify-end gap-6 items-center">
              <div className="flex items-center gap-2 group w-32">
                ...
                <div className="relative flex-1 h-1.5 bg-white/10 rounded-none flex items-center">
                  ...
                  <div className="h-full bg-slate-300 group-hover:bg-brand-gold transition-colors rounded-none" style={{ width: `${(isMuted ? 0 : volume) * 100}%` }} />
                </div>
              </div>
              <div className="h-6 w-px bg-white/10 hidden sm:block" />
              <div className="flex items-center gap-3">
                <div className="text-xs font-bold text-brand-gold hidden sm:block">
                  {Number(activeBeat.basicPrice ?? 0).toFixed(2)} € TTC
                </div>
...
function ActivePill({ label, onRemove, color }: {
  ...
  return (
    <button onClick={onRemove} className={`flex items-center gap-1.5 text-[11px] font-bold px-2.5 py-1 rounded-none border transition-all hover:opacity-75 ${colors[color]}`}>
