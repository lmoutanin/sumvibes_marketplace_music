...
const CAT: Record<string, { label: string; accent: string }> = {
  PRODUCTION:    { label: "Production",    accent: "#a855f7" },
  MARKETING:     { label: "Marketing",     accent: "#3b82f6" },
  EQUIPMENT:     { label: "Équipement",    accent: "#10b981" },
  COLLABORATION: { label: "Collaboration", accent: "#f59e0b" },
  OTHER:         { label: "Général",       accent: "#6b7280" },
};
        ...
        <div className="mx-auto max-w-3xl px-4 sm:px-6">
          ...
          {!loading && error && (
            <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-10 text-center">
              ...
              <Link href="/community/forum" className="btn-primary px-6 py-2.5 rounded-none text-sm font-semibold">
                ← Retour au forum
              </Link>
            </div>
          )}
              ...
              {!editing && (
                <div className="flex items-center gap-2 py-4 border-b border-white/8">
                  <button onClick={handleLike} disabled={!user || liking}
                    className={`flex items-center gap-2 px-4 py-2 rounded-none text-sm font-semibold transition-all border ${
                      liked
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 px-4 py-2 rounded-none text-sm font-semibold border border-white/10 hover:border-white/25 hover:bg-white/6 transition-all">
                    <MessageSquare className="w-4 h-4" />
                    <span>{comments.length} réponse{comments.length !== 1 ? "s" : ""}</span>
                  </button>

                  <div className="ml-auto flex gap-1">
                    <button className="p-2 rounded-none border border-white/10 hover:border-white/25 hover:bg-white/6 transition-all text-slate-400 hover:text-white" title="Partager">
                      <Share2 className="w-4 h-4" />
                    </button>
                    <button className="p-2 rounded-none border border-white/10 hover:border-white/25 hover:bg-white/6 transition-all text-slate-400 hover:text-white" title="Sauvegarder">
                          ...
                          <div key={c.id} className={`flex gap-4 py-5 ${i < pageComments.length - 1 ? "border-b border-white/5" : ""}`}>
                            ...
                            <div className="flex flex-col items-center gap-1 flex-shrink-0 pt-0.5">
                              ...
                              {i < pageComments.length - 1 && (
                                <div className="w-px flex-1 bg-white/5 min-h-[16px] rounded-none mt-1" />
                              )}
                            </div>
                      ...
                      {totalPages > 1 && (
                        <div className="flex items-center justify-center gap-2 mt-6 pt-5 border-t border-white/8">
                          ...
                          <button
                            ...
                            disabled={safePage === 1}
                            className="p-2 rounded-none border border-white/10 hover:border-white/25 hover:bg-white/6 transition-all text-slate-400 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed"
                ...
                <div id="comment-form" className="mt-8">
                  {user ? (
                      ...
                      <form onSubmit={handleComment}>
                        ...
                        <div className="flex items-center justify-between px-4 pb-3 pt-1">
                          ...
                          <button type="submit"
                            ...
                            className="btn-primary px-5 py-2 rounded-none text-sm font-semibold flex items-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed transition-all">
                    ...
                    <div className="rounded-2xl border border-white/10 p-8 text-center">
                      ...
                      <Link href="/auth/login" className="btn-primary px-6 py-2.5 rounded-none text-sm font-semibold">
                        Se connecter
                      </Link>
                    </div>
                  )}
