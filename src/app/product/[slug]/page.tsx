        ...
        <main className="pt-20 flex items-center justify-center min-h-[60vh]">
          <div className="text-center px-6">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link href="/catalogue" className="btn-primary px-8 py-3 rounded-none font-bold">Retour au catalogue</Link>
              <Link href="/" className="glass px-8 py-3 rounded-none font-bold hover:bg-white/10 transition-colors">Accueil</Link>
            </div>
          </div>
        </main>
      </div>
                  ...
                  <div className="relative aspect-square bg-gradient-to-br from-brand-purple/30 to-brand-pink/30 flex items-center justify-center overflow-hidden group">
                    ...
                    <div className={`absolute inset-0 flex items-center justify-center bg-black/20 group-hover:bg-black/40 transition-colors ${isPlayingAudio ? 'bg-black/40' : ''}`}>
                      ...
                      <button
                        ...
                        className={`absolute top-4 right-4 p-3 rounded-none glass border transition-all hover:scale-110 z-10 ${liked ? "text-rose-500 border-rose-500/50 bg-rose-500/10" : "text-white border-white/10 hover:border-white/30 bg-white/5"}`}
                      ...
                      <button
                        onClick={togglePlay}
                        className="rounded-none bg-brand-gold p-6 shadow-2xl hover:scale-110 transition-transform glow-gold text-black uppercase"
                      >
                        {isBuffering ? (
                ...
                <div className="mb-8">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {beat.instruments.map((inst) => (
                      <span key={inst} className="glass px-3 py-1.5 rounded-none text-sm">{inst}</span>
                    ))}
                  </div>
                </div>
              )}
